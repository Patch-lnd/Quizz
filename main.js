...
function displayNextQuestion() {
  if (answeredQuestions < questionsData.length) {
    let currentQuestionData = questionsData[answeredQuestions];
    ...
    appContainer.innerHTML = ""; // Clear previous question
    appContainer.appendChild(question.create());

    question.html.addEventListener("question-answered", ({ detail }) => {
      if (detail.answer.isCorrect) {
        score++;
      }

      answeredQuestions++;
      scoreContainer.innerHTML = `Score: ${score}/${answeredQuestions}`;

      if (answeredQuestions < questionsData.length) {
        setTimeout(displayNextQuestion, 500); // Transition to next question
      } else {
        setTimeout(() => {
